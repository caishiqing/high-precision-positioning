...
def compare_loss(pos1, pos2):
    ...
    dist = tf.keras.losses.mae(p1, p2) + 1e-9
    ...
    nd = tf.boolean_mask(nd, tf.less(nd, 5)) + 2e-2
...
def uniform_loss(pos, anchor_size=256, epsilon=1e-5):
    ...
    # gamma = tf.numpy_function(lambda *args: sinkhorn_stabilized(*args, reg=100, stopThr=1e-5, warn=False),
    #                           inp=[pos, anchor, dist],
    #                           Tout=tf.float32)
    # gamma /= tf.reduce_sum(gamma)
